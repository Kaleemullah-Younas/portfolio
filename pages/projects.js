        ...
        <div>
            ...
            <div className="w-screen h-auto space-y-16">
                ...
                <Container>
                    ...
                    <div className="w-full mt-2 mx-auto p-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-5">
                    <ProjectsCard />
                    </div>
    ...
    return (
        <>
            ...
            {projects.slice(0, 12).map((list, i) => (
                <div key={i} className="box w-full h-auto bg-dark-200 rounded-[5px] relative top-[50px] transition-all mb-[50px] mr-[5px] opacity-[.7] md:w-[265px] md:ml-0 hover:opacity-[1]">
...
function GithubRepo() {
    ...
    async function fetchRepos() {
        let res;
        let url = `https://api.github.com/users/${userInfo.full_name}/repos`
    ...
    return (
        <>
            {loading ? "Loading..." : error !== null ? <p>{error}</p> : (
                <>
                    {/* Modified to only show first 6 repos */}
                    {repos.slice(0, 8).map((rep, i) => (
                        <div key={i} className="relative w-full h-[180px] bg-dark-200 flex flex-col items-start justify-start px-4 py-3 mt-2 rounded-md md:w-[280px]">
                            ...
                            <a href={rep.html_url} target="_blank" className="absolute right-2 top-2 flex flex-row items-center">
                                <small className="underline">View</small>
                                <FaArrowRight className="ml-1 text-[10px] " />
                            </a>
                        </div>
                    ))}
                    {/* Added View More link if there are more than 6 repos */}
                    {repos.length > 8 && (
                        <div className="w-full text-center mt-8">
                            <a href={githubUrl} target="_blank" rel="noopener noreferrer" className="text-center text-green-200 underline">
                                View More
